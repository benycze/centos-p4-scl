...
ENV DEPS autoconf automake binutils bison flex gcc gcc-c++ gdb glibc-devel libtool make pkgconf pkgconf-m4 pkgconf-pkg-config redhat-rpm-config rpm-build rpm-sign strace asciidoc byacc ctags diffstat git intltool jna ltrace patchutils perl-Fedora-VSP perl-generators pesign source-highlight systemtap valgrind valgrind-devel cmake expect rpmdevtools rpmlint scl-utils scl-utils-build wget binutils cmake python2-pip python3-pip openssl-devel boost169-devel boosti169-static libffi-devel boost-devel python2-devel python36-devel Judy-devel libpcap-devel gc-devel

# Install all dependencies
RUN dnf update -y && dnf install -y epel-release && dnf config-manager --set-enabled PowerTools && dnf module -y enable mariadb-devel
