...
RUN dnf clean all && dnf upgrade -y &&  dnf update -y && dnf install -y epel-release dnf-plugins-core && dnf config-manager --set-enabled PowerTools && dnf module -y enable mariadb-devel && dnf install -y $DEPS
